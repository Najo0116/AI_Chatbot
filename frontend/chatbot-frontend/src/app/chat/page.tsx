...
export default function ChatPage() {
  ...
  async function onSend(e: React.FormEvent) {
    ...
    try {
      ...
      setMsgs(s => [...s, { id: crypto.randomUUID(), role: "assistant", text: `Error: ${msg || "failed"}` }]);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 flex flex-col gap-3 h-[90vh]">
      ...
      <form onSubmit={onSend} className="flex gap-2">
        ...
        <button className="rounded px-4 bg-black text-white disabled:opacity-60" disabled={busy || !input.trim()}>
          {busy ? "Thinking…" : "Send"}
        </button>
      </form>
    </div>
